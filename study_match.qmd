...
+ Predict into full-data feature set: 3 probabilities per participant, one per treatment (triplicate dataset and substitute treatments)
...
2. *Validity analysis*: compare sample-average observed abstinence (means of 0s and 1s) to predicted probabilities from RCT tx. This will show how well the model predictions match observed cases.
...
2/9/24: decided with John via Slack that we would do SHAP using the single, best selected model but using held-out folds from 1x 10-fold simple k-fold